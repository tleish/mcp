...
| `MCP_AUDIT_OUTPUT` | `file+stdout` | Audit output destination: `file` (ChronDB, queryable via `mcp logs`), `stdout`, `stderr` (JSON lines for container log drivers), `file+stdout`, `file+stderr` (ChronDB **and** JSON lines for both `mcp logs` and a log driver), or `none` (disable). Default is dual-sink: persists to ChronDB **and** mirrors entries as JSON on stdout. |
